...
该模块实现了华东地区风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
...
class huadong(BaseAccuracy):
    """华东地区风光功率预测精度评估类。
    
    该类实现了华东地区风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
    ...
    **kwargs : dict, optional
    ...
    def cdq_acc(self, da, cap, tag, label_column='real_value', pred_column='pred', delta=None, baseacc=None, **kwargs):
        ...
        mse = np.sqrt(np.mean(np.square((obs - pred) / cap)))
        acc = 1 - mse
