...
该模块实现了江苏地区风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
...
class jiangsu(BaseAccuracy):
    """江苏地区风光功率预测精度评估类。
    
    该类实现了江苏地区风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
    ...
    **kwargs : dict, optional
    ...
    def cdq_acc(self, da, cap, tag, label_column='real_value', pred_column='pred', delta=None, baseacc=None, **kwargs):
        ...
        err = abs(obs - pred) / cap
        err[err > 1] = 1
        acc = (1 - err) * 100 
        acc_temp = acc[acc < baseacc]
        score = len(acc_temp) * cap * 0.4     

        return {'acc': np.nanmean(acc), 'score': score}
    ...
    def dq_acc(self, da, cap, tag, label_column='real_value', pred_column='pred', **kwargs):
        ...
        if len(acc_temp) > len(obs) * 0.02:
            score = np.nansum(len(acc_temp) * cap * 1)
        else:
            score = 0
    ...
    def ust_day(self, pred: Dict[pd.Timestamp, pd.Series], obs: pd.Series, 
        ...
        **kwargs : dict, optional
        ...
        # 处理pred数据
        pred_df0 = pd.DataFrame(columns=['Datetime', 'pred'])
        pred_df16 = pd.DataFrame(columns=['Datetime', 'pred'])
        for pred_bj_date, pred_series in pred.items():
            try:
                if len(pred_series) == 16:
                    pred_time0 = pred_series.index[0]
                    pred_value0 = pred_series.iloc[0]
                    pred_df0.loc[len(pred_df0)] = [pred_time0, pred_value0]
                    pred_time16 = pred_series.index[-1]
                    pred_value16 = pred_series.iloc[-1]
                    pred_df16.loc[len(pred_df16)] = [pred_time16, pred_value16]
            except Exception as e:
                logger.error(f"处理预测日期 {pred_bj_date} 时出错: {str(e)}")
                continue

        pred_df0['Datetime'] = pred_df0['Datetime'] + pd.Timedelta(hours=8)  # 传过来的数据均为世界时, 按照业务需转为北京时
        pred_df16['Datetime'] = pred_df16['Datetime'] + pd.Timedelta(hours=8)  # 传过来的数据均为世界时, 按照业务需转为北京时
        dates = pred_df0['Datetime'].dt.date.unique()
        for date in dates:
            predi0 = pred_df0[pred_df0['Datetime'].dt.date == date]
            predi16 = pred_df16[pred_df16['Datetime'].dt.date == date]
            obsi = obs_df[obs_df['Datetime'].dt.date == date]
            predi0 = predi0.merge(obsi, on='Datetime', how='inner')
            predi16 = predi16.merge(obsi, on='Datetime', how='inner')
            date = pd.Timestamp(date, tz='Asia/Shanghai')
            if (len(predi0) == 0) or (len(predi16) == 0):
                logger.warning(f"在 {date} 没有匹配的观测数据")
                result[date] = {"acc": np.nan, "score": np.nan}
                continue
            acc0 = self.cdq_acc(predi0, cap, sta_type, label_column='power', pred_column='pred', baseacc=97)
            acc16 = self.cdq_acc(predi16, cap, sta_type, label_column='power', pred_column='pred', baseacc=87)
            acc = {'acc': np.nanmean([acc0['acc'], acc16['acc']]), 'score': np.nansum([acc0['score'], acc16['score']])}
