...
该模块实现了南网地区风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
...
class nanwang(BaseAccuracy):
    """南网地区风光功率预测精度评估类。
    
    该类实现了南网地区风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
    ...
    **kwargs : dict, optional
    ...
    def cdq_acc(self, da, cap, tag, label_column='real_value', pred_column='pred', delta=None, baseacc=None, **kwargs):
        ...
        cap_tmp = obs.copy()
        cap_tmp[obs < 0.2 * cap] = 0.2 * cap
        acc = 1 - np.sqrt(np.nansum(((obs - pred) / cap_tmp) ** 2) / len(obs))
        acc = np.round(acc * 100, 2)

        if tag == 'PV':
            score = (70 - acc) * cap * 1 * 1 if acc < 70 else 0
        elif tag == 'WD':
            score = (65 - acc) * cap * 0.2 * 1 if acc < 65 else 0
        else:
    ...
    def dq_acc(self, da, cap, tag, label_column='real_value', pred_column='pred', **kwargs):
        ...
        if tag == 'PV':
            score = (65 - acc) * cap * 1 * 1 if acc < 65 else 0
        elif tag == 'WD':
            score = (60 - acc) * cap * 0.2 * 1 if acc < 60 else 0
        else:
            logger.warning(f"未知站点类型：{tag}, score设为NaN")
            score = np.nan
        ...
        **kwargs : dict, optional
        ...
        for date in dates:
            ...
            if len(predi) == 0:
                ...
                continue
            acci = self.cdq_acc(predi, cap, sta_type, label_column='power', pred_column='pred')
            result.update({date: acci})
            logger.debug(f"评估完成 - 评估日期: {date}, 细则结果：{acci}")
