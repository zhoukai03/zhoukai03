...
该模块实现了行业标准风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
...
class nbtacc(BaseAccuracy):
    """行业标准风光功率预测精度评估类。
    
    该类实现了行业标准风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
    ...
    **kwargs : dict, optional
    ...
    def cdq_acc(self, da, cap, tag, label_column='real_value', pred_column='pred', delta=None, baseacc=None, **kwargs):
        ...
        nrmse = np.sqrt(np.nansum(((obs - pred) / cap) ** 2) / n)
        acc = (1 - nrmse) * 100
        ...
        **kwargs : dict, optional
        ...
        for date in dates:
            ...
            if len(predi) == 0:
                ...
                continue
            acci = self.nbtacc(predi, cap, sta_type, label_column='power', pred_column='pred')
        ...
        for pred_bj_date, pred_series in pred.items():
            try:
                if len(pred_series) >= 96:
                    ...
                    # 计算细则
                    acc = self.nbtacc(df_temp, cap, sta_type, label_column='power', pred_column='pred')
