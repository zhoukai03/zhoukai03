...
该模块实现了山东地区风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
...
class shandong(BaseAccuracy):
    """山东地区风光功率预测精度评估类。
    
    该类实现了山东地区风光功率预测的精度评估规范，支持超短期和短期预测的日评估和月评估。
    ...
    **kwargs : dict, optional
    ...
    def cdq_acc(self, da, cap, tag, label_column='real_value', pred_column='pred', delta=None, baseacc=None, **kwargs):
        ...
        for i in range(len(obs)):
            err = abs(obs[i] - pred[i])
            if err <= 0.15 * obs[i] and err <= 2:
                err = 0
            all_err.append(err)
            
        pe = np.nansum(np.array(all_err) * 0.25)

        score = pe * 0.02

        return {'acc': score, 'score': pe}
    ...
    def dq_acc(self, da, cap, tag, label_column='real_value', pred_column='pred', **kwargs):
        ...
        Args:
            ...
            pred_column (str, optional): 预测标签列名. Defaults to 'pred'.
        Returns:
            {acc (float): 扣分, score (float): 考核偏差电量}
        """
        obs = da[label_column].values
        pred = da[pred_column].values
