...
class TestCproducer:
    ...
    @patch('src.message.KafkaProducer')
    def test_send_meteo(self, mock_kafka_producer):
        """测试发送气象文件相关的消息"""
        mock_producer = MagicMock()
        mock_kafka_producer.return_value = mock_producer
        producer = Cproducer(["localhost:9092"])
        ...
        # 在运行时，pd.Timestamp和datetime是可以互相兼容的
        # 即使类型注解可能有差异，测试在实际运行时应该不会有问题
        with patch('pandas.Timestamp.strftime', return_value="2023-01-01 12:00:00"):
            producer.send_meteo("task123", "ST", "meteo.csv", pd.Timestamp("2023-01-01 12:00:00"))
