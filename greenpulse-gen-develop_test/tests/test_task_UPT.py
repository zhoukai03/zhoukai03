"""
Test suite for task.UPT (Update Training) functionality.
"""
...
from src.task import taskSingleTrain  # Reusing the same function as FT but with different parameters
...
def test_params():
    ...
    task = CParamsTask()
    task.taskType = TaskType.UPT  # Update training
    task.dateRange = ["2023-01-25", "2023-01-31"]  # One week of incremental data
    task.algorithm = {"catboost": ["v1.0"]}
    task.dataset = ["EC_IFS"]
    task.accuracy = ["mae"]
    task.postProcess = ["smoothing"]
    task.incremental = True  # Key difference from FT
...
# Unit Tests
def test_valid_update_training(test_params, test_logger, test_message_queue):
    """Test update training with valid input parameters."""
    ...
    with patch('src.task.dataLoader') as mock_loader, \
         ...
         patch('src.task.os.path.exists') as mock_exists:
        ...
        mock_model_instance.fit.assert_called()  # Should call fit for incremental training
        mock_dump.dump.assert_called()
        test_message_queue.send.assert_called()

# Integration Tests
@pytest.mark.integration
def test_complete_update_training_process(test_params, test_logger, test_message_queue):
    """Test complete update training process with all components."""
    init, task, sta, path = test_params
    checkpoint = datetime.now().strftime("%Y%m%d%H%M%S")
    ...
    with patch('src.task.dataLoader') as mock_loader, \
        ...
        # Verify model was trained incrementally
...
def test_update_training_model_handling(test_params, test_logger, test_message_queue, model_exists):
    """Test how update training handles existing and non-existing models."""
    init, task, sta, path = test_params
    checkpoint = datetime.now().strftime("%Y%m%d%H%M%S")
    ...
    with patch('src.task.dataLoader') as mock_loader, \
         ...
         patch('src.task.modelDump') as mock_dump, \
         patch('src.task.os.path.exists') as mock_exists, \
         patch('src.task.logging') as mock_logging:
        
        # Setup mock returns
        mock_loader.load.side_effect = [
            generate_mock_weather_data(task.dateRange[0], task.dateRange[1]),
            generate_mock_observation_data(task.dateRange[0], task.dateRange[1])
        ]
        ...
        # Verify appropriate logging based on model existence
        if model_exists:
            # Should proceed with incremental training
            mock_model_instance.fit.assert_called()
        else:
            # Should log a warning about model not found
